...
namespace Den.Tools.GUI
{
	public class UI
	{
		...
		{get{
			...
			//will not return true in some unknown cases (at least on Win in Unity 2020)
			
			#if !UNITY_EDITOR_OSX  //don't really know what on linux, just in case excluding only mac os
		...
		#region Draw
			...
			/// If calling two Draw instances in one window one should have offsetAfterDraw enabled (otherwise will not mouseUp)
			{
				...
				if (GUI.Draw.delayedCell!=null  &&  GUI.Draw.FieldLostFocus) 
					GUI.Draw.delayedCell = null;
