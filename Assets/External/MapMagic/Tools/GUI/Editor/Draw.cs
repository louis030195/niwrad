...
namespace Den.Tools.GUI
{
	public static class Draw
	{
		...
		public static Cell delayedCell;
		private static float delayedFloat;
		...
		#region Fast Fields
			...
			public static float Field (float val, GUIStyle style=null) 
			{
				...
				Rect rect = Cell.current.GetRect(UI.current.scrollZoom, padding:fieldPadding);
				float newVal = EditorGUI.FloatField(rect, val, style: (style!=null ? style : UI.current.styles.field) );
				...
				{
					delayedFloat = newVal;
					delayedCell = Cell.current;
				}

				if (delayedCell == Cell.current  &&  (FieldLostFocus || UI.current.editorWindow != EditorWindow.focusedWindow))
				{
		...
		#region Class
			...
			/// If cellFields defined fills cell to fieldInfo dictionary
			{
				...
				for (int a=0; a<attributes.Length; a++)
				{
					...
					try { ClassField(att.field, att.type, obj, att.name, att.min, att.max, att.isLeft, att.allowSceneObject); }
			...
			public static void ClassField (
				FieldInfo field, Type type, object obj, string name, 
				float min = float.MinValue, float max = float.MaxValue, 
				bool isLeft=false, bool allowSceneObject=false)
			{
				...
				if (type == typeof(float))
				{
					...
					{
						float srcVal = Getter<float>.GetGetter(field)(obj);
						float dstVal = Field(srcVal, name);
						if (dstVal < min) dstVal = min;
						if (dstVal > max) dstVal = max;
						if (Cell.current.valChanged)  field.SetValue(obj, dstVal); 
					}
				}
