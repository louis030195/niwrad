...
namespace Den.Tools.Matrices
{
	...
	public class MatrixWorld : Matrix, ICloneable
	{
		...
		public Vector2D PixelSize => new Vector2D(worldSize.x/(rect.size.x-1), worldSize.z/rect.size.z);
		//public Vector2D PixelSize => new Vector2D(worldSize.x/(rect.size.x-1), worldSize.z/(rect.size.z-1)); //this should be right, but had not tested
